...
@section('content')
@php($support = $message) {{-- Alias to avoid clashing with Blade's @error $message --}}
    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-gray-800 rounded-lg p-6 border border-gray-700 mb-6">
            <div class="flex items-center space-x-4 mb-4">
                <h2 class="text-2xl font-semibold text-cyan-400">{{ $support->subject }}</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-gray-300 text-sm mb-6">
                <div>
                    ...
                    <p class="font-medium">{{ $support->user->name }}</p>
                    <p class="text-gray-400">{{ $support->user->email }}</p>
                </div>
                <div>
                    <span class="text-gray-400">Related Event:</span>
                    <p>{{ $support->event ? $support->event->title : 'General inquiry' }}</p>
                    @if($support->event)
                        <p class="text-gray-400">{{ $support->event->event_date->format('M d, Y') }}</p>
                    @endif
                </div>
                <div>
                    <span class="text-gray-400">Submitted:</span>
                    <p>{{ $support->created_at->format('M d, Y H:i') }}</p>
                </div>
            </div>

            <div class="bg-gray-700 rounded p-4">
                <h3 class="text-cyan-300 font-medium mb-2">User Message:</h3>
                <p class="text-gray-200 whitespace-pre-wrap">{{ $support->message }}</p>
            </div>
        </div>
            ...
            <div class="bg-cyan-900 rounded-lg p-6 border border-cyan-700 mb-6">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-cyan-400 text-sm">
                        by {{ $support->admin->name ?? 'Admin' }} -
                        {{ optional($support->admin_responded_at)->format('M d, Y H:i') }}
                    </span>
                </div>
                <p class="text-cyan-100 whitespace-pre-wrap">{{ $support->admin_response }}</p>
            </div>
        @endif
        ...
        <div class="bg-gray-800 rounded-lg p-6 border border-gray-700">
            <h3 class="text-xl font-semibold text-cyan-300 mb-4">
                {{ $support->admin_response ? 'Update Response' : 'Send Response' }}
            </h3>

            <form action="{{ route('admin.support.respond', $support->id) }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    @error('status')
                        <p class="text-red-400 text-sm mt-1">{{ $errors->first('status') }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="admin_response"
                              ...
                              required
                              class="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                              placeholder="Type your response to the user...">{{ old('admin_response', $support->admin_response) }}</textarea>
                    @error('admin_response')
                        <p class="text-red-400 text-sm mt-1">{{ $errors->first('admin_response') }}</p>
                    @enderror
                </div>
